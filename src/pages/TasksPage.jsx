...
export function TasksPage({ platformData, onStartTask, onOpenProfile, submittingTaskId, mode = 'claim', onSubmitOrderInfo }) {
  ...
  const viewTasks = claimedTasks
    .map((claim) => tasks.find((task) => task.id === claim.taskId))
    .filter(Boolean)

  const visibleTasks = mode === 'view'
    ? (viewTasks.length ? viewTasks : [])
    ...
    <div className="space-y-4">
      <section>
        <p className="mb-3 text-xs uppercase tracking-[0.28em] text-slate-400">
          {mode === 'view' ? 'Task Center' : 'Task Hall'}
        </p>

        {visibleTasks.length === 0 ? (
          <div className="rounded-[24px] bg-white px-4 py-8 text-center text-sm text-slate-500 shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
            {mode === 'view' ? 'No claimed tasks yet.' : 'No available task right now.'}
          </div>
        ) : (
            ...
            {visibleTasks.map((task) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    disabled={(mode === 'claim' && locked) || submitting}
