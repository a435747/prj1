...
app.post('/api/platform/claim-task', frontendAuth, (req, res) => {
  ...
  const exists = (req.frontendUser.platformData.claimedTasks ?? []).some(
    (item) => item.taskId === taskId && item.status !== '已拒绝' && item.status !== 'rejected',
  )
...
app.post('/api/platform/task-claims/:claimId/submit', frontendAuth, (req, res) => {
  ...
  const nextDb = updateDb((current) => {
    ...
    const nextClaims = (nextUsers[ui].platformData.claimedTasks ?? []).map((claim) => {
      ...
      if (!['待提交','pending_proof','已拒绝','rejected'].includes(claim.status)) return claim
...
app.post('/api/admin/task-claims/:claimId/review', adminAuth, (req, res) => {
  ...
  const db = updateDb((current) => {
    ...
    updatedClaim = { ...nextClaims[tci], status: action === 'approve' ? '已完成' : '已拒绝', reviewedAt: formatTimestamp(), summary: action === 'approve' ? `${nextClaims[tci].title} approved.` : `Rejected: ${String(reason||'Resubmit clearer proof.').trim()}` }
...
app.post('/api/admin/withdraw-requests/:requestId/review', adminAuth, (req, res) => {
  ...
  const db = updateDb((current) => {
    ...
    updatedRequest = { ...nextReqs[tri], status: action === 'approve' ? '已完成' : '已拒绝', reviewedAt: formatTimestamp(), summary: action === 'approve' ? 'Withdrawal approved.' : `Rejected: ${String(reason||'Check account details.').trim()}` }
...
app.post('/api/admin/verifications/:verificationId/review', adminAuth, (req, res) => {
  ...
  const db = updateDb((current) => ({
    ...
    frontendUsers: (current.frontendUsers ?? []).map((u) => {
      ...
      updatedVerification = { ...u.verification, status: action === 'approve' ? '已完成' : '已拒绝', reviewedAt: formatTimestamp(), summary: action === 'approve' ? 'Identity verification approved.' : `Rejected: ${String(reason||'Resubmit documents.').trim()}` }
...
app.post('/api/platform/recharge-requests', frontendAuth, (req, res) => {
  ...
  if (!user.verification || (user.verification.status !== 'approved' && user.verification.status !== '已完成')) {
...
app.post('/api/admin/orders/:orderNo/review', adminAuth, (req, res) => {
  ...
  const { action } = req.body
  if (!['approve','reject'].includes(action)) return res.status(400).json({ message: 'Invalid action.' })
  const db = updateDb((current) => ({ ...current, adminData: { ...current.adminData, orders: (current.adminData.orders ?? []).map((o) => o.orderNo === orderNo ? { ...o, status: action === 'approve' ? '已完成' : '已拒绝', finalPayTime: formatTimestamp(), paid: action === 'approve' ? 'Yes' : 'No' } : o) } }))
